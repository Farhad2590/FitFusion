...
const NutritionTracker = () => {
  ...
  const getData = async () => {
    ...
    setError(null);
    try {
      const response = await axios.get(`http://localhost:9000/nutration/${user?.email}`);
      ...
      weeklyMeals.forEach(dayData => {
        if (dayData && dayData.day && newWeeklyData[dayData.day]) {
  ...
  const saveDaily = async () => {
    ...
    try {
      ...
      await axios.post(`http://localhost:9000/meals`, mealData);
    } catch (error) {
      ...
      <div className="max-w-4xl mx-auto space-y-6">
        ...
        <div className="bg-gray-900 rounded-lg p-6 border border-red-500">
          <div className="flex justify-between items-center mb-6">
            <div className="flex items-center space-x-3">
              <FaCalendar className="text-red-500 w-6 h-6" />
              <h2 className="text-2xl font-bold text-white">{days[selectedDay]} Tracking</h2>
            </div>
            <button
              onClick={saveDaily}
                ...
                <div className="max-h-60 overflow-y-auto">
                  {filteredFoods.map(food => (
                    ...
                    >
                      ...
                      <div className="flex items-center space-x-3">
                      <span className="text-red-500">{commonFoods[food].calories} cal</span>
                      <FaStar
                        className={`w-5 h-5 cursor-pointer ${favorites.includes(food) ? 'text-yellow-400' : 'text-gray-600'}`}
                        onClick={(e) => {
                          e.stopPropagation();
                          toggleFavorite(food);
                        }}
        ...
        <div className="space-y-4">
          {meals[days[selectedDay]].name.map((meal, index) => (
            <div key={index} className="flex justify-between items-center p-3 bg-black rounded-lg border border-red-500">
              ...
              <div className="flex items-center space-x-3">
                <span className="text-red-500">{meals[days[selectedDay]].calories[index]} cal</span>
                <button
        ...
        <div className="mt-6 p-4 bg-black rounded-lg border border-red-500">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div className="bg-gray-900 p-3 rounded-lg">
              ...
              <div className="text-red-500 text-xl font-bold">{totals.calories}</div>
            ...
            <div className="bg-gray-900 p-3 rounded-lg">
              ...
              <div className="text-blue-500 text-xl font-bold">{totals.protein}g</div>
            </div>

            <div className="bg-gray-900 p-3 rounded-lg">
              <div className="text-gray-400">Carbs</div>
              <div className="text-green-500 text-xl font-bold">{totals.carbs}g</div>
            </div>

            <div className="bg-gray-900 p-3 rounded-lg">
              <div className="text-gray-400">Fat</div>
              <div className="text-yellow-500 text-xl font-bold">{totals.fat}g</div>
            </div>
          </div>
        </div>
      </div>
        ...
        <div className="text-white">
          ...
          <div className="mt-6 grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="bg-black p-4 rounded-lg border border-red-500">
              <div className="text-gray-400">Weekly Average</div>
            ...
            <div className="bg-black p-4 rounded-lg border border-red-500">
              <div className="text-gray-400">Highest Day</div>
              <div className="text-xl font-bold text-red-500">
            ...
            <div className="bg-black p-4 rounded-lg border border-red-500">
              <div className="text-gray-400">Days On Target</div>
              <div className="text-xl font-bold text-red-500">
